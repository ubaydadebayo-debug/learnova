...
import { listQuizzes } from '../../services/adminService';
  ...
  return (
    <section className="container-page py-8">
      <div className="mb-8">
        ...
        <h1 className="mt-1 text-2xl font-extrabold sm:text-3xl">Quiz oversight</h1>
        <p className="mt-1 text-navy/60">Review every quiz on the platform and its attempt activity.</p>
      </div>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search quiz, course, module or instructor…"
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-line bg-white p-12 text-center">
          <Trophy className="mx-auto mb-4 h-10 w-10 text-primary/50" />
        ...
        <div className="overflow-x-auto rounded-2xl border border-line bg-white shadow-sm">
          <table className="w-full min-w-[820px] text-left text-sm">
            ...
            <tbody className="divide-y divide-line">
              {quizzes.map((quiz) => (
                <tr key={quiz.id} className="hover:bg-surface/40">
                  <td className="px-4 py-3">
                    <p className="font-semibold">{quiz.title}</p>
                    <span className="text-xs text-navy/50">
                      Pass {quiz.passingScore}% · {quiz.allowRetake ? 'Retakes allowed' : 'Retakes off'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <Link to={`/course/${quiz.module.course.slug}`} className="block max-w-[240px] truncate font-semibold hover:text-primary" title={quiz.module.course.title}>
                      {quiz.module.course.title}
                    </Link>
                    <span className="text-xs text-navy/50">{quiz.module.title}</span>
                  </td>
                  <td className="px-4 py-3">
                    <p className="font-semibold">{quiz.module.course.instructor.firstName} {quiz.module.course.instructor.lastName}</p>
                    <span className="text-xs text-navy/50">{quiz.module.course.instructor.email}</span>
                  </td>
                  <td className="px-4 py-3 text-center text-navy/70">{quiz.questions}</td>
                  <td className="px-4 py-3 text-center text-navy/70">{quiz.attempts}</td>
                  <td className="px-4 py-3 text-center">
                    {quiz.averagePercentage !== null ? (
                      <span className={`inline-flex min-w-[48px] justify-center rounded-full px-2 py-0.5 text-xs font-semibold ${quiz.averagePercentage >= quiz.passingScore ? 'bg-success/10 text-success' : 'bg-error/10 text-error'}`}>
          ...
          {pagination.totalPages > 1 && (
            <div className="flex items-center justify-between gap-4 border-t border-line px-4 py-3 text-sm text-navy/60">
              <span>Page {pagination.page} of {pagination.totalPages} · {pagination.total} quizzes</span>
